...
export default function BlogListingClient({ posts, categories }: Props) {
  ...
  const featured = useMemo(() => filteredPosts.filter((p) => p.featured), [filteredPosts]);

  return (
    <div className="container-max py-8">
      {/* Header */}
      <div className="text-center mb-12">
        <h1 className="text-4xl lg:text-5xl font-bold text-secondary-500 mb-4">Financial Education Blog</h1>
        <p className="text-xl text-secondary-100 max-w-3xl mx-auto">
          Expert insights, tips, and guides to help you make smarter financial decisions. From budgeting basics to advanced investment strategies, we've got you covered.
        </p>
      </div>
            ...
            {featured.map((post) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <h3 className="text-xl font-bold text-secondary-500 mb-3 leading-tight">{post.title}</h3>

                  <p className="text-secondary-100 mb-4 leading-relaxed">{post.excerpt}</p>
          ...
          {filteredPosts.map((post) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <h3 className="text-lg font-bold text-secondary-500 mb-2 leading-tight">{post.title}</h3>

                <p className="text-secondary-100 mb-3 text-sm leading-relaxed line-clamp-3">{post.excerpt}</p>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <Link href={`/blog/${post.slug}`} className="text-primary-500 hover:text-primary-600 font-medium">
                    Read →
                  </Link>
                </div>
              </div>
